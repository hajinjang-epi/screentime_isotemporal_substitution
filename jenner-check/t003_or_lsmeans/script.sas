/* Adapted from analysis_cross-sectional.sas (hajinjang-epi/screentime_isotemporal_substitution)
   The %or macro is the paper's Table 2 model: a logistic regression of obesity
   on a categorical total-screen-time exposure (total_re, quartiles), reporting
   adjusted odds ratios via LSMEANS with a Bonferroni-adjusted CL. Model spec,
   class refs, and macro body are kept verbatim; only the input rows below are
   synthetic (mock exposure/demographic rows in place of the KCYPS2018 survey
   extract the original script reads via a local OneDrive libname). */
