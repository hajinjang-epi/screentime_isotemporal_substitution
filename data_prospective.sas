libname a 'C:\Users\HajinJang\OneDrive - University of Pittsburgh\PITT\ARCHIVES\SCREENTIME\0_230201_screen_diet\KCYPS2018_all';
data kcyps18_e4;
...
/*주관적 건강상태*/
if YPHY1A00w1=1 then subhealthw1=1; *very poor;
else if YPHY1A00w1=2 then subhealthw1=2; *poor;
else if YPHY1A00w1=3 then subhealthw1=3; *good;
else if YPHY1A00w1=4 then subhealthw1=4; *very good;
else if YPHY1A00w1=. then subhealthw1=9; *missing;

if subhealthw1 in (1,2,3,4) then do;
if subhealthw1 in (1,2) then subhealthw1_bin=1; *poor;
else if subhealthw1 in (3,4) then subhealthw1_bin=2; *good;
end;
...
data cohort; set e4; run;



/****************************************/
/* prospective: incidence odds ratios (2018) */
/****************************************/

/* Exclusion */
data cohort1; set cohort;
if heightw1=. or weightw1=. then delete;
run;
* 3951 -> 2594;
...
data cohort4; set cohort3;
if ctime8w1=. or ctime7w1=. or sleep_all_h=. or ctime11w1=. or ctimestudyw1=. or ctime2w1=. or ctimetstw1=. or ctime10w1=. or ctimespw1=. then delete;
run;  
* 2325;

data cohort5; set cohort4;
if obesityw4 in (0,1);
run; 
* 2325 -> 2023;
...
/* obesity incidence between 2018-2021 */
data cohort6; set cohort5;
if obesityw2=1 or obesityw3=1 or obesityw4=1 then obe_inc=1;
else obe_inc=0;
run;
...
* pros_e4;
data pros_e4; set cohort6;; run; *2023;


/* category-specific median values for p-trend 
%macro med(data,ex,time);
data &data.; set &data.; dayuse_screen=sum(dayuse10w1, dayuse9w1, sp_usew1); run;
proc sort data=&data.; by &ex.; run;
proc means data=&data. median;
by &ex.; var &time.; run;
%mend med;

%med(pros_pool,total_re,dayuse_screen);
%med(pros_pool,sp_new,sp_usew1);
%med(pros_pool,com_new,dayuse9w1);
%med(pros_pool,tv_new,dayuse10w1);

%med(pros_e4,total_re,dayuse_screen);
%med(pros_e4,sp_new,sp_usew1);
%med(pros_e4,com_new,dayuse9w1);
%med(pros_e4,tv_new,dayuse10w1);

%med(pros_m1,total_re,dayuse_screen);
%med(pros_m1,sp_new,sp_usew1);
%med(pros_m1,com_new,dayuse9w1);
%med(pros_m1,tv_new,dayuse10w1); 
*/


*e4;
data pros_e4; set pros_e4;
dayuse_screen=sum(dayuse10w1, dayuse9w1, sp_usew1); 
if 72<dayuse_screen<73 then med_e4_combined=1;
else if 143<dayuse_screen<144 then med_e4_combined=2;
else if 197<dayuse_screen<198 then med_e4_combined=3;
else if 300<=dayuse_screen<301 then med_e4_combined=4;

if 0<=sp_usew1<1 then med_e4_sp=1;
else if 36<=sp_usew1<37 then med_e4_sp=2;
else if 90<=sp_usew1<91 then med_e4_sp=3;
else if 150<=sp_usew1<151 then med_e4_sp=4; 
else if 227<=sp_usew1<228 then med_e4_sp=5; 

if 0<=dayuse9w1<1 then med_e4_com=1;
else if 15<=dayuse9w1<16 then med_e4_com=2;
else if 90<=dayuse9w1<91 then med_e4_com=3;
else if 150<=dayuse9w1<151 then med_e4_com=4;

if 0<=dayuse10w1<1 then med_e4_tv=1;
else if 36<=dayuse10w1<37 then med_e4_tv=2;
else if 90<=dayuse10w1<91 then med_e4_tv=3;
else if 150<=dayuse10w1<151 then med_e4_tv=4;
run;
...
ctimespw1=ctimespw1*60;
ctime9w1=ctime9w1*60;
ctime10w1=ctime10w1*60;
ctimetstw1=ctimetstw1*60;

/* smartphone */
*spfinal;
if ctimespw1<60 then spfinal=1;
else if 60<=ctimespw1<120 then spfinal=2;
else if 120<=ctimespw1<180 then spfinal=3;
else if ctimespw1>=180 then spfinal=4;
*comfinal;
if ctime9w1<60 then comfinal=1;
else if 60<=ctime9w1<120 then comfinal=2;
else if ctime9w1>=120 then comfinal=3;
*tvfinal;
if ctime10w1<60 then tvfinal=1;
else if 60<=ctime10w1<120 then tvfinal=2;
else if 120<=ctime10w1<180 then tvfinal=3;
else if ctime10w1>=180 then tvfinal=4;
*totalfinal;
if ctimetstw1<120 then sumfinal=1;
else if 120<=ctimetstw1<180 then sumfinal=2;
else if 180<=ctimetstw1<240 then sumfinal=3;
else if ctimetstw1>=240 then sumfinal=4;
run;


proc freq data=pros_e4; table spfinal comfinal tvfinal sumfinal; run;
